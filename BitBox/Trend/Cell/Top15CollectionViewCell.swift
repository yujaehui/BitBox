...
class Top15CollectionViewCell: BaseCollectionViewCell {
    ...
    func configureCell(row: Int, currentCell: Item) {
        ...
        priceLabel.text = NumberFormatterManager.shared.dollarFormatter(currentCell.data.price)
        priceChangePercentageLabel.text = NumberFormatterManager.shared.percentageFormatter(currentCell.data.priceChangePercentage24H["usd"] ?? 0)
        priceChangePercentageLabel.textColor = currentCell.data.priceChangePercentage24H["usd"] ?? 0 >= 0 ? ColorStyle.red : ColorStyle.blue
    }
}
